...
outfile_name="./examples/fgsm/logits.txt"
    ...
    for n, (x, y) in enumerate(dataloaders['test']):
        ...
        for i, attacked_model in enumerate(models):
            ...
            for j in range(0, x.shape[0]):
                save_image(x[j], f'./examples/fgsm/{j}_{model_names[i]}_originalREAL_{y[j].item()}_CLEAN_{out[j].item()}_ADV_{out_perturbed[j].item()}.png')
                save_image(perturbed_x[j], f'./examples/fgsm/{j}_{model_names[i]}_perturbedREAL_{y[j].item()}_CLEAN_{out[j].item()}_ADV_{out_perturbed[j].item()}.png')
                save_image(perturbed_x[j]-x[j], f'./examples/fgsm/{j}_{model_names[i]}_deltaREAL_{y[j].item()}_CLEAN_{out[j].item()}_ADV_{out_perturbed[j].item()}.png')
