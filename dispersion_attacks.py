...
from utils.adversarial.attack import kpixel_attack, mean_std_distance
from torchvision import transforms


outfile_name="./attack_results/dispersion.txt"
    ...
    for i, attacked_model in enumerate(models):
        ...
        for j, (x, y) in enumerate(dataloaders['test']):
            ...
            for idx, k in enumerate(kappa):
                m,s=mean_std_distance(kpixel_attack(x, perturbed_x, k=k)-x)
                mean[idx]+=m
                std[idx]+=s
        with open(outfile_name, 'a') as outfile:
            outfile.write("\nAttack on "+model_names[i]+" mean distances: "+str(mean/len(dataloaders['test'])))
            outfile.write("\nAttack on "+model_names[i]+" mean distance std: "+str(std/len(dataloaders['test'])))
        print('model: ', model_names[i], " mean: ", mean/len(dataloaders['test']))
        print('model: ', model_names[i], " mean std: ", std/len(dataloaders['test']))
